...
Handy Openbox desktop wickedness on a Debian system you can run with **just like that** after a net install and never have to worry about this sort of thing again. Best for a fresh install, but, scripts can be run any time and with subsequent users. This desktop setup is a pretty good way to go if you enjoy the Linux. It has been reliable anf functional for me for a long time. It's out of your way, light, fast, boring - no whiz-bang at all, like, no ricing, or, whatever.
